...
| Domain logic | backend `app/domain/` 골격 생성, 실제 cohort/privacy logic은 다음 단계 |
| Storage | 계획됨 |
| Tests | backend health test와 frontend lint/build 확인 시작 |
| Docs workflow | 생성 완료 |
...
이 구조는 방향이다. 현재 scaffold는 실행 골격과 health check까지 완료했고, 다음 단계에서 domain model, privacy guard, ingestion module을 실제 구현한다.
