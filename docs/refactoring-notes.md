...
| Frontend code | Next.js scaffold 생성 완료 |
| Backend code | FastAPI scaffold와 `/health` 생성 완료 |
| Domain logic | i485tracker-like normalized `TimelineRecord`, `CohortSummary`, `SourceRecord`, `PrivacyRule` 및 cohort/privacy helper 구현 완료 |
| Storage | 계획됨 |
| Tests | backend health, domain model, privacy/cohort tests 확인 |
| Docs workflow | 생성 완료 |
...
이 구조는 방향이다. 현재 scaffold는 실행 골격, health check, backend domain model, privacy guard, cohort summary helper까지 완료했고, 다음 단계에서 template checkpoint와 ingestion module을 진행한다.
