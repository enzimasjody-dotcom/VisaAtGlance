...
| Frontend code | Next.js scaffold와 public overview preview 첫 화면 구성 |
| Backend code | FastAPI scaffold, `/health`, public dashboard aggregate API 구현 |
| Domain logic | i485tracker-like `TimelineRecord`, public aggregate, privacy guard, cohort helper 구현 |
| Ingestion | local mock fixture와 ignored full-data cache 기반 validation gate 완료 |
| Storage | anonymous `CaseRecord + CaseEvent` 저장 구조는 Phase 4B 후보 |
| Tests | backend health, domain model, ingestion, privacy/cohort, public dashboard API tests 확인 |
| Docs workflow | 문서 주인, diff 승인, PR 절차 기준 정리 |
...
이 구조는 방향이다. 현재 scaffold, health check, backend domain model, privacy guard, cohort summary helper, i485tracker-like ingestion prototype, public dashboard aggregate API까지 완료했다. 다음 구조 작업은 Phase 4A public overview frontend 구현 이후, Phase 4B에서 anonymous `CaseRecord + CaseEvent` foundation으로 진행한다.
