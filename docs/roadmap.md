...
### Phase 4 — public dashboard MVP + cohort context + anonymous case record

Phase 4는 제품 검증 흐름을 기준으로 `4A -> 4C-1 -> 4B -> 4C-2` 순서로 진행한다.

먼저 공개 dashboard가 어떤 가치를 주는지 보여주고, 그 다음 cohort context의 chart/metric contract를 확정한다. 이후 anonymous case record 저장 기반을 만들고, 마지막으로 실제 사용자 case를 cohort context에 연결한다.

#### Phase 4A — Public Overview MVP
...
| public chart/metric contract | 첫 화면 chart와 metric이 무엇을 의미하고 무엇을 의미하지 않는지 정리 | 3개 chart set과 안전 문구 확정. approved-only 평균/중앙값을 public hero metric으로 쓰지 않음 |
| dashboard preview QA | frontend 첫 화면이 legal-safe copy와 기존 CTA 구조를 유지하는지 확인 | `make visa-frontend-check` 통과 |

Phase 4A chart set은 다음 세 가지로 시작한다.

| Dashboard label | 기반 분석 output | 역할 |
|---|---|---|
| `Filing-month cohort movement` | `filing_month_approval_rate.png` | filing month별 observed movement와 approval wave 확인 |
| `Community timeline progression` | `km_approval_curve.png` | pending case를 포함한 community timeline 진행 흐름 확인 |
| `Approved-case processing distribution` | `approval_time_histogram.png` | 승인 기록이 있는 case의 과거 처리기간 분포 확인 |

#### Phase 4C-1 — Cohort Context Contract / Read-only Prototype

| 작업 | 내용 | 완료 기준 |
|---|---|---|
| filing-month cohort movement contract | filing month 기준 cohort movement를 public chart 후보로 정의 | 필요한 입력 필드, sample size 기준, limitation 문구 확정 |
| pending-aware timeline analysis 후보 검토 | Kaplan-Meier류 접근 등 pending-aware 분석 후보를 검토 | 통계적 정확성 확인 전 production metric으로 쓰지 않음 |
| user-entered context input contract | 사용자가 입력해야 할 최소 필드 정의 | category, filing date, service/field office 등 cohort context 필드 확정 |
| legal-safe language contract | cohort context 표현 기준 정리 | approval prediction이나 legal advice 없이 historical community-data position만 표시 |

#### Phase 4B — Anonymous Case Record Foundation
...
#### Phase 4C-2 — User-entered Cohort Context Implementation

| 작업 | 내용 | 완료 기준 |
|---|---|---|
| cohort context API/UI 연결 | 사용자가 입력하거나 저장한 timeline을 비슷한 filing-month/category/field office cohort 흐름 안에서 설명 | approval prediction 없이 similar case timeline context 표시 |
| timeline dashboard | i485tracker-like normalized timeline aggregate statistics와 chart | filing-month cohort movement, pending-aware timeline analysis, cohort context를 MVP 핵심 축으로 표시 |
...
기술적 복잡성, 확장성, 데이터 안정성과 제품 검증 흐름을 함께 고려해 Phase 4 구현 순서는 다음과 같이 둔다.

1. Phase 4A: public overview MVP
2. Phase 4A: source/limitation 표시
3. Phase 4A: public chart/metric contract
4. Phase 4C-1: filing-month cohort movement contract
5. Phase 4C-1: pending-aware timeline analysis 후보 검토
6. Phase 4C-1: user-entered context input contract
7. Phase 4B: `CaseRecord + CaseEvent` backend domain model과 테스트
8. Phase 4B: anonymous case creation API와 private edit token 발급/검증
9. Phase 4B: edit link 기반 case event update API
10. Phase 4B: duplicate candidate detection의 보수적 후보 탐지
11. Phase 4B: aggregate-only contribution pipeline과 privacy guard 연결
12. Phase 4C-2: user-entered timeline cohort context UI/API 연결

이 순서의 이유:

- public overview가 먼저 있어야 사용자가 사이트의 가치를 입력 없이 이해할 수 있다.
- cohort context contract가 먼저 정리되어야 어떤 사용자 입력 필드와 차트가 필요한지 알 수 있다.
- case/event 모델은 contract 이후 구현해 불필요한 저장 필드를 줄인다.
- private edit token은 로그인 없는 업데이트 경험의 핵심이므로 auth보다 먼저 검증한다.
- duplicate detection은 자동 병합이 아니라 후보 제안으로 시작해 데이터 손상 위험을 낮춘다.
- pending-aware 분석과 실제 사용자 case 연결은 통계 방식과 저장 모델이 확인된 뒤 production 기능으로 붙인다.
