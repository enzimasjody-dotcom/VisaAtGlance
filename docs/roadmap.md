...
| 3 | i485tracker-like mock timeline ingestion prototype | 진행 중 |
...
| quality report | local full-data cache와 mock fixture의 missing field, invalid date, cohort 규모를 점검 | dashboard 개발 전에 데이터 품질 리스크 확인 |
| data validation gate | full-data cache 품질 리포트, invalid row sample, raw-to-normalized mapping, small cohort suppression 기준을 리뷰 | public dashboard MVP 착수 전에 승인된 validation 기준 확보 |
| import validation | 필수 field, ISO date format, 비현실적 year 처리 기준 | 잘못된 case를 조용히 섞지 않음 |
...
| timeline dashboard | i485tracker-like normalized timeline aggregate statistics와 chart | 공개 사용자가 기본 trend 확인 가능 |
| anonymous case input | normalized timeline 필드와 호환되는 형식으로 사용자가 자기 case 입력 | 로그인 없이 입력 가능 |
