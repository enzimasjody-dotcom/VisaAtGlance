...
| 2 | backend domain model과 privacy guard foundation | 예정 |
| 2.1 | template checkpoint 생성 | 예정 |
| 3 | Google Sheet timeline ingestion prototype | 예정 |
| 4 | public dashboard MVP + anonymous timeline contribution | 예정 |
| 5 | AdSense 필수 페이지 | 예정 |
...
- Google Sheet timeline import는 실제 데이터 구조를 가장 빨리 검증할 수 있으므로 USCIS/Visa Bulletin 전체 pipeline보다 먼저 둔다.
- public dashboard는 ingestion prototype과 privacy 기준이 있어야 신뢰 가능한 형태로 확장된다.
- Phase 4부터 사용자가 입력한 비식별 timeline data를 동의 기반으로 저장하고, privacy guard를 통과한 aggregate stats에 반영한다.
- 개별 사용자 입력 row는 public row로 바로 공개하지 않고, 평균 처리시간, similar case count, percentile, recent approval trend 같은 집계 지표에만 사용한다.
- AdSense 필수 페이지는 dashboard MVP 뒤에 두어 단순 페이지 껍데기가 아니라 original dashboard와 함께 검토 가능한 사이트가 되게 한다.
- sign up/auth는 saved timeline 최소 기능과 함께 도입해 가입할 이유와 저장할 데이터 단위를 명확히 한다.
...
| 기본 QA | `make visa-check`, `make visa-frontend-check`, `make visa-backend-check` 명령 정리 | README와 Git workflow에 반영 |

### Phase 2 — backend domain model과 privacy guard

| 작업 | 내용 | 완료 기준 |
|---|---|---|
| domain model foundation | `TimelineRecord`, `CohortSummary`, `SourceRecord`, `PrivacyRule` backend 모델 초안 | backend domain test로 기본 shape 검증 |
| privacy guard foundation | small cohort suppression, row-level 공개 제한 기준 구현 | 작은 sample에서 percentile 숨김 테스트 통과 |
| cohort summary helper | average, median, percentile 후보 계산을 privacy guard 뒤에 둠 | 작은 sample에서는 percentile이 숨겨짐 |
| template checkpoint | Phase 2 완료 상태를 재사용 가능한 app foundation으로 기록 | tag 또는 template branch와 문서 기록 생성 |

### Phase 3 — spreadsheet ingestion prototype

| 작업 | 내용 | 완료 기준 |
|---|---|---|
| Google Sheet import prototype | Google Sheet 기반 timeline 데이터를 읽고 정규화하는 경로 결정 | 샘플 데이터로 local transform 가능 |
| source metadata | 데이터 출처, checked date, limitation 표시 구조 | dashboard API가 source/freshness metadata 반환 |
| import validation | 필수 column, date format, empty value 처리 기준 | 잘못된 row를 조용히 섞지 않음 |
| aggregate-first output | row-level 원본 대신 public dashboard용 집계 구조 생성 | public chart가 aggregate data만 사용 |

### Phase 4 — public dashboard MVP + anonymous timeline contribution

| 작업 | 내용 | 완료 기준 |
|---|---|---|
| public overview | 핵심 지표와 public chart 첫 화면 | 로그인 없이 주요 지표 확인 가능 |
| spreadsheet dashboard | Google Sheet 기반 aggregate statistics와 chart | 공개 사용자가 기본 trend 확인 가능 |
| anonymous case input | spreadsheet 구조와 동일하거나 호환되는 필드로 사용자가 자기 case 입력 | 로그인 없이 입력 가능 |
| instant comparison | 평균 처리시간, similar case count, percentile, recent approval trend 표시 | small cohort 기준 통과 시만 상세 지표 표시 |
| anonymous contribution | 사용자가 동의하면 비식별 timeline data를 저장해 aggregate stats에 반영 | 개별 row 공개 없이 aggregate 계산에만 사용 |
...
| protected dashboard shell | 로그인 사용자 영역 | 저장 기능이 들어갈 자리 제공 |
| saved timelines 최소 모델 | Phase 4에서 입력한 timeline 또는 saved filter 저장 | 재방문 시 저장된 항목 복원 |
...
- **공개/로그인 분리**: 공개 사용자는 기본 statistics, public charts, aggregate trends와 일회성 timeline comparison을 사용할 수 있고, 로그인 사용자는 saved timelines, advanced filtering, alerts를 사용한다.
- **동의 기반 익명 기여**: Phase 4부터 사용자가 입력한 비식별 timeline data는 명시적 동의 후 저장해 aggregate stats에 반영할 수 있다.
