...
| 1.1 | frontend/backend 초기 scaffold | 예정 |
...
| 기본 scaffold 생성 | `frontend/` Next.js와 `backend/` FastAPI 생성. frontend 첫 화면, backend `/health` 포함 | 두 dev server 실행 가능 |
...
| 기본 QA | frontend typecheck/lint/build와 backend test 명령 정리 | `README.md`와 `git-workflow.md`에 반영 |
