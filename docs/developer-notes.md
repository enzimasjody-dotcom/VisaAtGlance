...
| `UserCase` | 로그인 사용자의 저장된 관심 조건과 claim한 case 목록 |
| `CaseRecord` | 사용자가 생성하거나 claim한 I-485 case의 안정적인 기본 record |
| `CaseEvent` | case 진행 중 계속 업데이트되는 timeline event |
| `CaseEditToken` | 로그인 없이 case를 수정할 수 있는 private edit link 권한 |
...
| Timeline Input | cohort context 확인 | filing date, category, office 기준 community timeline context |
...
User Value Section
  Enter timeline -> cohort context / community timeline position -> save or claim with login
