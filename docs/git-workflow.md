...
| 항목 | 포함할 내용 |
|---|---|
| 큰 그림 목표 | 장기 제품/기술 방향에서 왜 필요한지 |
| 작은 그림 목표 | 이번 MR 또는 이번 작업에서 무엇을 끝낼지 |
...
계획된 도구나 기술이 설치되어 있지 않거나 바로 사용할 수 없을 때, 임의로 대체 구현을 선택하지 않는다.

규칙:

- `uv`, package manager, framework generator, hosting CLI, database, auth provider처럼 workflow나 구조에 영향을 주는 도구가 없으면 먼저 사용자에게 알린다.
- 대안(`venv + pip`, 다른 package manager, 수동 scaffold 등)을 적용하기 전에 사용자 확인을 받는다.
- 단순 조회 명령, 일회성 확인 명령, 문서-only 작업처럼 구조 결정에 영향이 없는 경우는 예외로 둘 수 있다.
- 대안을 선택했다면 이유와 되돌릴 수 있는 방법을 작업 요약에 남긴다.

## 불확실성 처리 원칙

API 동작, 라이브러리 버전별 인터페이스, 데이터 타입, 외부 서비스 동작, 브라우저/런타임 차이처럼 확신이 없는 부분은 추측으로 구현하지 않는다.

규칙:

- 모르면 먼저 “모른다, 확인이 필요하다”고 말한다.
- 공식 문서, 로컬 코드, 타입 정의, 테스트, 실제 실행 결과 중 하나로 확인한 뒤 구현한다.
- 확인하지 못한 가정은 제안서와 작업 요약에 명시한다.
- 추측 구현 후 롤백하는 비용이 큰 영역은 사용자 확인 없이 진행하지 않는다.
- 특히 API contract, auth/session, storage schema, migration, dependency version interface, external data shape는 확인 우선 영역으로 본다.

## UI/정보구조 변경 승인 원칙

기존 화면의 구조, 섹션 순서, 정보 우선순위, 카피, 사용자가 보는 주요 지표를 크게 바꾸는 작업은 단순 구현이 아니라 제품/UX 설계 변경으로 본다.

규칙:

- 기존 페이지 구조를 유지하라는 명시가 없더라도, API 연결이나 데이터 교체 작업에서는 기본적으로 기존 layout과 정보구조를 보존한다.
- hero, dashboard preview, navigation, section 구성, CTA, 주요 metric, chart 종류를 크게 바꾸기 전에 별도 제안과 사용자 승인을 받는다.
- “더 유용할 것 같다”는 판단만으로 정보 항목을 새로 만들거나 기존 정보를 제거하지 않는다.
- 데이터가 아직 작거나 mock이면, 화면에 표시할 정보의 의미와 한계를 먼저 설명하고 승인받는다.
- 사용자가 승인한 범위를 넘어 UI를 재설계했다면 커밋하지 않고 즉시 멈춘 뒤 되돌림 또는 축소안을 제안한다.
...
푸쉬 또는 PR 머지 후에는 저장소 상태와 계획 문서를 다시 정렬한다.

규칙:

- GitHub Issues와 repo의 모든 `.md` 파일을 검토하고 최신 상태로 업데이트한다.
- 새 코드, 제품 결정, QA 결과, 운영 절차가 문서와 충돌하면 후속 문서 업데이트 PR을 만든다.
- 로드맵은 고정된 순서가 아니라 현재 상황에 맞게 재정렬한다.
- 로드맵 우선순위는 기술적 복잡성, 확장성, 장기계획을 모두 고려해 정한다.
- 우선순위가 바뀌면 [로드맵](roadmap.md)에 이유를 짧게 남긴다.
- GitHub Issues가 제품/기술 계획과 충돌하거나 오래되었으면 issue 업데이트, close, 새 issue 생성 중 하나를 제안한다.
...
| Frontend 변경 | `make visa-frontend-check` |
| Backend 변경 | `make visa-backend-check` |
| 전체 scaffold/check | `make visa-check` |
